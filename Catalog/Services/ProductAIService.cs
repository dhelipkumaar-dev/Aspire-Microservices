...
)
{
    public async Task<string> SupportAsync(string userQuery, CancellationToken cancellationToken = default)
    {
        ...
        var productCatalog = products.Count == 0
    ...
    private async Task InitEmbeddingsAsync()
    {
        ...
        {
            var productInfo =
                $"[{product.Name}] is a product that costs [{product.Price}] and is described as [{product.Description}]";
    ...
    {
        if (!await productVectorCollection.CollectionExistsAsync()) await InitEmbeddingsAsync();
